...
describe("Discover", function () {
  ...
  let should = chai.should()
  const prefixes = ["6011", "65", "644", "645", "646", "647", "648", "649"]

  for(let i=0; i < prefixes.length; i++) {
    const prefix = prefixes[i]

    let shortCardLength = 0;
    let longCardLength = 0;

    if (prefix.length === 4) {
      shortCardLength = 123412341234
      longCardLength = 123451234512345
    } else if (prefix.length === 2) {
      shortCardLength = 12341234123412
      longCardLength = 12345123451234512
    } else if (prefix.length === 3) {
      shortCardLength = 1234123412341
      longCardLength = 1234512345123451
    }

    it(`has a prefix of ${prefix} and a length of 16`, function () {
      should.equal(detectNetwork(prefix + `${shortCardLength}`), "Discover");
    });
